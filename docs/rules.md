...
2. Все кредитные предложения должны храниться в отдельных файлах в директории `src/lib/models/creditOffers/`
   - Каждое предложение должно быть в своем файле, например `ferratum-quick.ts`
   - Именование файлов должно следовать формату `lenderId-creditType.ts`
   - Индексный файл должен экспортировать все предложения и предоставлять функции для их фильтрации
3. Такая структура обеспечивает:
   - Единый источник данных для каждого типа кредита и кредитного предложения
   - Возможность детального описания каждого типа и предложения
   - Легкое масштабирование при добавлении новых типов или предложений
